...
## v3.7.1 更新亮点（2026-08-01）

本次更新把 Civitai 同步相关功能接入统一网络层，修复联网与缓存问题，并完善预设路由。

### Civitai 同步联网修复
- `Lora Data Preview` 和顶部 `NAIBA` 入口的元数据查询、图片下载、批量同步、预设校验统一接入 `naiba_network.py`，支持自动 / 手动 / 直连代理及连接测试。
- `Lora Data Preview` 节点新增 `proxy_mode` / `proxy_url` 输入参数，可在节点上直接指定代理策略（不再依赖全局设置）。
- 校验结果区分「本地+C站」「仅本地」「查询失败」等状态，网络失败不再被误报为绿色通过或未上站。
- 图片 CDN 失败会显示明确错误并保留重试机会；旧版无图失败缓存会自动迁移，强制刷新失败也不会覆盖已有完整元数据。
- 修复顶部 `NAIBA` 入口同步成功后访问空节点对象、反而显示错误的问题。
- 新增 `tests/test_civitai_utils.py`，覆盖代理上下文、错误分类、预览重试、缓存保留和图片格式校验。

### 预设路由与预览前端
- `preset_routes.py` 扩展预设校验 / 同步路由逻辑，配合统一网络层处理代理与错误返回。
- `js/lora_data_preview.js` 与 `lora_data_preview.py` 适配代理参数与更明确的校验状态展示。
...
**磁盘缓存系统**：支持将预览图和搜索结果缓存到本地磁盘，减少重复网络请求。缓存默认关闭，可在设置面板开启。支持后台预加载（持续循环缓存预览图直到空间用满），浏览器关闭时自动停止预加载。
...
| use_disk_cache | BOOLEAN | True | 开启磁盘缓存（预览图 + 搜索结果写入本地缓存目录） |
| max_cache_mb | INT | 1200 | 磁盘缓存上限（MB），到限自动 LRU 淘汰 |
...
#### 新增功能
- **Lora Data Preview 预设管理入口**：弹窗头部「Lora Data Preview - Civitai 同步」标题与「网格」切换之间新增「预设管理」按钮，一键打开 LoRA 预设管理弹窗（列出 / 加载 / 保存 / 删除 / 重命名 / 导出 / 导入）
- **预设管理弹窗兼容无节点上下文**：NAIBA 工具栏入口以 `node=null` 打开时，自动查找画布首个 `LoraDataPreview` 节点以获得完整功能；保存 / 导出 / 从文件导入 / 导入到节点在无 `LoraDataPreview` 节点时给出明确提示而非静默失败

#### 优化改进
- **保存预设改为自定义对话框**：从原生 `prompt` 改为自定义弹窗，默认预填当前选中的预设名；名称命中已有预设（大小写不敏感）时显示「将覆盖现有内容」警告，主按钮变为「确认覆盖」，需二次点击才落盘
- **预设列表可取消选中**：点击已选中的预设可再次点击取消选中（`selectedPreset = null`），方便另存为新预设

#### 节点改进
- **List LoRA Loader `clip` 输入可选化**：`clip` 由必填改为可选（归入 `optional`），不连接时只加载模型侧 LoRA，与 Multi LoRA Loader 行为对齐
...
#### 问题修复
- **缓存状态获取失败**：修复 `cache_status` 接口访问不存在的 `_DISK_CACHE._index` 属性导致 500 错误，改为遍历目录统计文件数
- **设置面板遗留文本**：移除设置面板底部遗留的 "节点上的「随机生成」按钮会自动同步到弹窗扭蛋结果（双向同步）" 叙述
